...
exports.save = async (req, res) => {
  try {
    ...
    for (const g of grades) {
      ...
      if (existing) {
        await run("UPDATE grades SET class_score=?, exam_score=?, grade=?, remarks=?, recorded_by=? WHERE id=?",
          [g.class_score, g.exam_score, grade, remarks, req.user?.id, existing.id]);
      } else {
        await run("INSERT INTO grades (student_id, subject, class, term, academic_year, class_score, exam_score, grade, remarks, recorded_by) VALUES (?,?,?,?,?,?,?,?,?,?)",
...
exports.getStudentReport = async (req, res) => {
  ...
  const totalEnrollment = await get("SELECT COUNT(*) as count FROM students WHERE class=? AND status='active'", [student.class])?.count;
...
exports.topStudents = async (req, res) => {
  ...
  const top = await query(sql, params);
  res.json(top);
};
