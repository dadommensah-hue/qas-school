...
exports.sendFeeReminders = async (req, res) => {
  try {
    const { term } = req.body;
    const defaulters = await query(`SELECT s.full_name, s.guardian_phone, s.guardian_name, f.amount, f.amount_paid, f.amount-f.amount_paid as balance
...
exports.logs = async (req, res) => {
  ...
  const logs = await query(`SELECT * FROM sms_logs ORDER BY sent_at DESC LIMIT ?`, [parseInt(limit) || 50]);
  res.json(logs);
};
