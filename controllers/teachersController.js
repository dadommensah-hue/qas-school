...
exports.list = (req, res) => {
  try {
    const teachers = query(`SELECT t.*, GROUP_CONCAT(DISTINCT ts.subject) as assigned_subjects 
...
exports.create = async (req, res) => {
  try {
    ...
    const c = get("SELECT COUNT(*) as c FROM teachers").c;
    ...
    run(`INSERT INTO teachers (teacher_id,full_name,gender,phone,email,subject_specialization,qualification,profile_photo,username,password) VALUES (?,?,?,?,?,?,?,?,?,?)`,
...
exports.get = (req, res) => {
  try {
    const t = get("SELECT * FROM teachers WHERE id=?", [req.params.id]);
    if (!t) return res.status(404).json({ error: 'Not found' });
    const subjects = query("SELECT id,subject,class FROM teacher_subjects WHERE teacher_id=?", [req.params.id]);
    res.json({ ...t, subjects });
  } catch (e) { res.status(500).json({ error: e.message }); }
};
...
exports.update = (req, res) => {
  try {
    ...
    if (photo) {
      run(`UPDATE teachers SET full_name=?,gender=?,phone=?,email=?,subject_specialization=?,qualification=?,status=?,profile_photo=? WHERE id=?`,
        [full_name, gender, phone, email, subject_specialization, qualification, status||'active', photo, req.params.id]);
    } else {
      run(`UPDATE teachers SET full_name=?,gender=?,phone=?,email=?,subject_specialization=?,qualification=?,status=? WHERE id=?`,
...
exports.delete = (req, res) => {
  try {
    run("DELETE FROM teacher_subjects WHERE teacher_id=?", [req.params.id]);
    run("DELETE FROM teachers WHERE id=?", [req.params.id]);
    res.json({ message: 'Teacher deleted' });
  } catch (e) { res.status(500).json({ error: e.message }); }
};

exports.assignSubjects = (req, res) => {
  try {
    const { subjects } = req.body;
    run("DELETE FROM teacher_subjects WHERE teacher_id=?", [req.params.id]);
    for (const s of (subjects||[])) {
      run("INSERT INTO teacher_subjects (teacher_id,subject,class) VALUES (?,?,?)", [req.params.id, s.subject, s.class||'all']);
    }
...
exports.assignClass = (req, res) => {
  try {
    ...
    run("INSERT OR IGNORE INTO teacher_class_assignments (teacher_id, assigned_class) VALUES (?,?)", [req.params.id, assigned_class]);
...
exports.getClassAssignments = (req, res) => {
  try {
    const assignments = query(`SELECT tca.id, tca.assigned_class, t.full_name, t.teacher_id 
      FROM teacher_class_assignments tca JOIN teachers t ON tca.teacher_id=t.id ORDER BY tca.assigned_class`);
    res.json(assignments);
  } catch (e) { res.status(500).json({ error: e.message }); }
};

exports.removeClassAssignment = (req, res) => {
  try {
    run("DELETE FROM teacher_class_assignments WHERE id=?", [req.params.id]);
...
exports.getMyClasses = (req, res) => {
  try {
    ...
    const classes = query("SELECT * FROM teacher_class_assignments WHERE teacher_id=?", [userId]);
    res.json(classes);
  } catch (e) { res.json([]); }
};
