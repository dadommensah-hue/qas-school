...
exports.getStudentReport = async (req, res) => {
  ...
  // Check approval
  const approval = await get("SELECT id FROM approved_terms WHERE class=? AND term=? AND exam_type='midterm' AND academic_year=?",
...
exports.approve = async (req, res) => {
  try {
    ...
    const existing = await get("SELECT id FROM approved_terms WHERE class=? AND term=? AND exam_type='midterm' AND academic_year=?", [cls, term, ay]);
    if (!existing) {
