...
exports.upload = (req, res) => {
  try {
    ...
    const u = get("SELECT full_name FROM users WHERE id=?", [req.user.id]);
    run("INSERT INTO syllabus (title,subject,class,filename,filepath,filesize,mimetype,uploaded_by,uploader_name) VALUES (?,?,?,?,?,?,?,?,?)",
...
exports.download = (req, res) => {
  try {
    const s = get("SELECT * FROM syllabus WHERE id=?", [req.params.id]);
...
exports.delete = (req, res) => {
  try {
    ...
    run("DELETE FROM syllabus WHERE id=?", [req.params.id]);
