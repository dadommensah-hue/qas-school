...
exports.getStudentResults = async (req, res) => {
  try {
    ...
    const scores = await query("SELECT * FROM mock_exam WHERE student_id=? AND (mock_number=? OR mock_number IS NULL) ORDER BY subject",
      [student_id, mock_number]);
    const withGrades = scores.map(s => ({ ...s, grade: beceGrade(parseFloat(s.score)||0) }));
...
exports.getClassResults = async (req, res) => {
  try {
    ...
    const results = await Promise.all(students.map(async st => {
      const rawScores = await query("SELECT * FROM mock_exam WHERE student_id=? AND (mock_number=? OR mock_number IS NULL)",
        [st.id, mock_number]);
...
exports.saveScores = async (req, res) => {
  try {
    ...
    for (const s of (scores||[])) {
      if (MOCK_SUBJECTS.indexOf(s.subject) === -1) continue;
      const grade = beceGrade(parseFloat(s.score)||0);
      const stype = CORE.includes(s.subject) ? 'core' : 'elective';
      const existing = await get("SELECT id FROM mock_exam WHERE student_id=? AND subject=? AND mock_number=?",
        [s.student_id, s.subject, mn]);
      if (existing) {
        await run("UPDATE mock_exam SET score=?,grade=?,subject_type=? WHERE id=?",
          [s.score, grade, stype, existing.id]);
      } else {
        await run("INSERT INTO mock_exam (student_id,subject,class,score,grade,subject_type,mock_number) VALUES (?,?,?,?,?,?,?)",
...
exports.deleteStudentScores = async (req, res) => {
  try {
    const mock_number = parseInt(req.query.mock_number) || 1;
    await run("DELETE FROM mock_exam WHERE student_id=? AND (mock_number=? OR mock_number IS NULL)",
      [req.params.student_id, mock_number]);
...
exports.getMockList = async (req, res) => {
  try {
    const rows = await query("SELECT DISTINCT mock_number FROM mock_exam ORDER BY mock_number");
