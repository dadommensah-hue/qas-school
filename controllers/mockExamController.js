...
function calcAggregate(scores) {
  const withGrades = scores.map(s => ({ ...s, grade: s.grade || beceGrade(parseFloat(s.score)||0) }));
...
exports.getStudentResults = async (req, res) => {
  try {
    ...
    const scores = await query("SELECT * FROM mock_exam WHERE student_id=? AND (mock_number=? OR mock_number IS NULL) ORDER BY subject", [student_id, mock_number]);
    const withGrades = scores.map(s => ({ ...s, grade: s.grade || beceGrade(parseFloat(s.score)||0) }));
...
exports.saveScores = async (req, res) => {
  try {
    ...
    for (const s of (scores||[])) {
      ...
      if (existing) await run("UPDATE mock_exam SET score=?,grade=?,subject_type=? WHERE id=?", [s.score, grade, stype, existing.id]);
      else await run("INSERT INTO mock_exam (student_id,subject,class,score,grade,subject_type,mock_number) VALUES (?,?,?,?,?,?,?)",
...
exports.deleteStudentScores = async (req, res) => {
  try {
    const mock_number = parseInt(req.query.mock_number) || 1;
    await run("DELETE FROM mock_exam WHERE student_id=? AND (mock_number=? OR mock_number IS NULL)", [req.params.student_id, mock_number]);
