  ...
  try {
    ...
    if (portal === 'student') {
      ...
      if (t && t.password && await bcrypt.compare(password, t.password)) {
        ...
        const assignedClassesRaw = await query("SELECT assigned_class FROM teacher_class_assignments WHERE teacher_id=?", [t.id]);
        const assignedClasses = assignedClassesRaw.map(r => r.assigned_class);
...
exports.me = async (req, res) => {
  try {
    ...
    if (portal === 'student') {
      ...
      const assignedClassesRaw = await query("SELECT assigned_class FROM teacher_class_assignments WHERE teacher_id=?", [id]);
      const assignedClasses = assignedClassesRaw.map(r => r.assigned_class);
